...
Let $O \subseteq \{1..n\} \times \{1..m\}$ be the set of observed (non-zero) entries in $X$. The ridge-regularized objective function is:
...
\mynote{
\textbf{About RMSE computation}
Test RMSE is computed only on user–item pairs seen in training (in our split there are no cold-start pairs), and predictions are rounded to the allowed rating set.
}
...
\begin{equation}
    Pr(X_{t+1} = A \mid X_t = B) = \frac{Pr(X_{t+1} = A, X_t = B)}{Pr(X_t = B)} \approx \frac{N_{BA}}{N_B}
\end{equation}
...
\begin{equation}
    Pr(X_5 = A \mid X_4 = B) = \hat{p}_{BA} = \frac{3}{5} = 0.6
\end{equation}
...
\underline{Task 4 - Solution}. Let $X_t \in \{A, B\}$ denote the grade in semester $t$. We model grades as a time-homogeneous second-order Markov chain:

\begin{equation}
    Pr(X_{t+1} = z \mid X_t = y, X_{t-1} = x) = \theta_{xy \rightarrow z}, \quad x,y,z \in \{A,B\}
\end{equation}

For Sam we need 

\begin{equation}
    Pr(X_5 = A \mid X_4 = B, X_3 = A) = \theta_{AB \rightarrow A}
\end{equation}

since Sam's history is $X_1 = B$, $X_2 = A$, $X_3 = A$, $X_4 = B$.

[MLE of the 2nd-order transition]

From the training rows only (X1-X8), form all adjacent triples
$(X_t, X_{t+1}, X_{t+2})$ for $t=1,2,3$ in each row.

For each context pair $(x,y)$, count 

\begin{equation}
    N_{xy \rightarrow z} = \#\{t : X_t = x, X_{t+1} = y, X_{t+2} = z\}, \quad N_{xy} = \sum_{z \in \{A,B\}} N_{xy \rightarrow z}
\end{equation}

The MLE is

\begin{equation}
    \hat{\theta}_{xy \rightarrow z} = \frac{N_{xy \rightarrow z}}{N_{xy}}
\end{equation}

Equivalently, by Bayes' rule,

\begin{equation}
    Pr(X_{t+2} = z \mid X_{t+1} = y, X_t = x) = \frac{Pr(x,y,z)}{Pr(x,y)} \approx \frac{N_{xy \rightarrow z}}{N_{xy}}
\end{equation}

[Counts for the needed context $(A,B)$]

From the table (rows $X_1$ to $X_8$ only), the triples give 

\begin{equation}
    N_{AB \rightarrow A} = 6, \quad N_{AB \rightarrow B} = 3, \quad N_{AB} = 9
\end{equation}

Hence 

\begin{equation}
    Pr(X_5 = A \mid X_4 = B, X_3 = A) = \hat{\theta}_{AB \rightarrow A} = \frac{6}{9} = \frac{2}{3} \approx 0.6667
\end{equation}
